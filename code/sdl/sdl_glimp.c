...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction) {
	...
	for (i = 0; i < 16; i++) {
	  ...
	  // If not allowing software GL, demand accelerated
		...
		if ((SDL_window = SDL_CreateWindow(CLIENT_WINDOW_TITLE, x, y, glConfig.vidWidth, glConfig.vidHeight, flags)) ==
			NULL) {
		...
		if (fullscreen) {
			...
			case 16:
				...
				break;
			case 24:
				sdlmode.format = SDL_PIXELFORMAT_RGB24;
				break;
			default:
			...
			sdlmode.w = glConfig.vidWidth;
			sdlmode.h = glConfig.vidHeight;
			sdlmode.refresh_rate = glConfig.displayFrequency = ri.Cvar_VariableIntegerValue("r_displayRefresh");
			sdlmode.driverdata = NULL;

			if (SDL_SetWindowDisplayMode(SDL_window, &sdlmode) < 0) {
...
success:
	...
	if (qglGetStringi) {
		...
		Q_strncpyz(glConfig.extensions_string, (const char *)qglGetString(GL_EXTENSIONS),
...
void GLimp_EndFrame(void) {
	...
	if (r_fullscreen->modified) {
		...
		fullscreen = (SDL_GetWindowFlags(SDL_window) & (SDL_WINDOW_FULLSCREEN | SDL_WINDOW_FULLSCREEN_DESKTOP)) != 0;
