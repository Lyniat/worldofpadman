...
void SV_GetChallenge(netadr_t from) {
	...
	NET_OutOfBandPrint(NS_SERVER, challenge->adr, "challengeResponse %d %d %d", challenge->challenge, clientChallenge,
...
static int SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
	...
	if (!cl->download) {
		...
		if (!(sv_allowDownload->integer & DLF_ENABLE) || (sv_allowDownload->integer & DLF_NO_UDP) || idPack ||
			unreferenced || (cl->downloadSize = FS_SV_FOpenFileRead(cl->downloadName, &cl->download)) < 0) {
...
void SV_UserinfoChanged(client_t *cl) {
	...
	if (strlen(val)) {
		...
		else if (i > 400 || strlen(val) > 4) {
			Info_SetValueForKey(cl->userinfo, "handicap", "400");
		}
	}
...
static qboolean SV_ClientCommand(client_t *cl, msg_t *msg) {
	...
	if (!com_cl_running->integer && cl->state >= CS_ACTIVE && sv_floodProtect->integer &&
		...
		// TTimo - moved the ignored verbose to the actual processing in SV_ExecuteClientCommand, only printing if the
		// core doesn't intercept
		clientOk = qfalse;
	}
...
static void SV_UserVoip(client_t *cl, msg_t *msg, qboolean ignoreData) {
	...
	for (i = 0, client = svs.clients; i < sv_maxclients->integer; i++, client++) {
		...
		client->voipPacket[(client->queuedVoipIndex + client->queuedVoipPackets) % ARRAY_LEN(client->voipPacket)] =
			packet;
...
void SV_ExecuteClientMessage(client_t *cl, msg_t *msg) {
	...
	if (serverId != sv.serverId && !*cl->downloadName && !strstr(cl->lastClientCommandString, "nextdl")) {
		if (serverId >= sv.restartedServerId &&
			serverId < sv.serverId) { // TTimo - use a comparison here to catch multiple map_restart
